...
EXEC_TIME=$(get_env "PHP_MAX_EXECUTION_TIME" "3600")

echo "🚀 Iniciando phpMyAdmin em http://127.0.0.1:$PORT..."
echo "⚙️ Configurações PHP: Upload Max=$UPLOAD_LIMIT | Memory Limit=$MEMORY_LIMIT | Exec Time=$EXEC_TIME"

php -d upload_max_filesize=$UPLOAD_LIMIT \
    -d post_max_size=$POST_LIMIT \
    -d memory_limit=$MEMORY_LIMIT \
    -d max_execution_time=$EXEC_TIME \
